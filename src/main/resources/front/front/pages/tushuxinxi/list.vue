	...
	<view class="content">
		<view :style='{"padding":"160rpx 0 160rpx","borderColor":"#21d5ae","alignItems":"flex-start","display":"block","justifyContent":"space-between","minHeight":"100vh","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20221223/869602be22f342bf8136d5a9f62af2f7.gif) no-repeat right top,url(http://codegen.caihongy.cn/20221223/a3dca8908c4c40b68f285ba0a3226a99.png) no-repeat left top / 100% auto,url(http://codegen.caihongy.cn/20221223/5100c3e4960949eeaacbc47dc102c484.png) no-repeat left bottom / 100% auto,#fff","borderWidth":"2rpx 0","width":"100%","position":"relative","borderStyle":"dashed","height":"auto"}'>
			...
			<view :style='{"width":"100%","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
				<scroll-view scroll-y="true" class="category-two" :style='{"minHeight":"100vh","padding":"24rpx 8rpx 0","borderColor":"#1bd0a9","margin":"20rpx 0 0","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,.1) 0%, rgba(143,232,213,.1) 80%, rgba(27,208,169,.1) 100%)","borderWidth":"0px 0px 0px 0","width":"220rpx","borderStyle":"dashed dashed dashed dashed","height":"100%"}'>
					<view :class='categoryName === item.tushufenlei ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.tushufenlei)">{{item.tushufenlei}}</view>
				</scroll-view>
				...
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"56rpx 20rpx 56rpx","margin":"0 0 20rpx","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20221223/eeb1d8003bcf41b0acd7cd88f736e734.png) no-repeat left top / 100% auto,url(http://codegen.caihongy.cn/20221223/bc76a8b5867243299dc80d79ed1de844.png) no-repeat left bottom / 100% auto,rgba(238,238,238,.6)","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
				  ...
				  <view class="listmr" :style='{"width":"calc(100% - 200rpx)","padding":"0","margin":"0","height":"auto"}'>
					...
					<view :style='{"width":"100%","padding":"16rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('tushuxinxi','修改')) || (!userid && isAuthFront('tushuxinxi','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('tushuxinxi','删除')) || (!userid && isAuthFront('tushuxinxi','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"2rpx solid #fff","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#1bd0a9","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"9999"}' v-if="userid && isAuth('tushuxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"2rpx solid #fff","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#1bd0a9","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"9999"}' v-if="!userid && isAuthFront('tushuxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('tushufenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('tushufenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,tushufenlei:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.tushufenlei = '%' + this.categoryName + '%'
				}
